...
namespace Punga\Plugin\System\PungaAnalytics\Extension;
...
use Punga\Plugin\System\PungaAnalytics\Service\CountryDatabaseMatcher;
 ...
 */
final class PungaAnalytics extends CMSPlugin implements SubscriberInterface
{
	 ...
	 * @return void
	 */
	public function onPungaAnalyticsRecord(Event $event): void
	{
		$this->recordCustomEvent($event, 'pungaanalytics_recorded');
	}

	/**
	 * Accepts the pre-0.7 event name so existing integrations keep working.
	 *
	 * @param Event $event Joomla event.
	 *
	 * @return void
	 *
	 * @deprecated 0.7.0 Dispatch onPungaAnalyticsRecord instead.
	 */
	public function onSimpleStatsRecord(Event $event): void
	{
		$this->recordCustomEvent($event, 'simplestats_recorded');
	}

	/**
	 * Records a custom event and writes its acknowledgement argument.
	 *
	 * @param Event  $event              Joomla event.
	 * @param string $acknowledgementKey Result argument name.
	 *
	 * @return void
	 */
	private function recordCustomEvent(Event $event, string $acknowledgementKey): void
	{
		try
		{
			$event->setArgument($acknowledgementKey, $this->collectCustomEvent($event->getArguments()));
		}
		catch (\Throwable $exception)
		{
			$event->setArgument($acknowledgementKey, false);
			$this->logFailure($exception);
		}
	}
	...
	private function collectPageView(): void
	{
		...
		$excludedComponents = $this->parseCommaList((string) $params->get('exclude_components', 'com_ajax,com_users,com_pungaanalytics'));
	...
	{
		Log::add(
			'Punga Analytics collection failed: ' . $exception->getMessage(),
			Log::WARNING,
			'plg_system_pungaanalytics'
		);
	}
}
