...
final class StatisticsArchiveService
{
	...
	private function acquireLock(): string
	{
		...
		$lockName = 'simplestats_' . substr(hash('sha256', $table), 0, 48);
		$db->setQuery('SELECT GET_LOCK(' . $db->quote($lockName) . ', 10)');

		if ((int) $db->loadResult() !== 1)
		{
			throw new \RuntimeException('Could not acquire the Simple Stats maintenance lock.');
		}
	...
	): void
	{
		...
		$db = $this->database;
		$eventsTable = $db->quoteName($db->replacePrefix('#__simplestats_events'));
		$dimensionsTable = $db->quoteName($db->replacePrefix('#__simplestats_daily_dimensions'));
		$label = '`' . $labelColumn . '`';
	...
	private function archiveEventItems(string $cutoffDate): void
	{
		$db = $this->database;
		$eventsTable = $db->quoteName($db->replacePrefix('#__simplestats_events'));
		$itemsTable = $db->quoteName($db->replacePrefix('#__simplestats_daily_items'));
